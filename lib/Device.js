...
class Device
{
  ...
  writeEnergyMeter() // eslint-disable-line complexity
  {
    ...
    const changes = this.changes;
    const modelChanged = changes.model !== undefined;
    const cnt = state.unit === 2;

    let i = MSD_FIRST_REGISTER_INDEX;

    if (modelChanged || changes.constant !== undefined)
    {
    ...
    if (modelChanged || changes.unit !== undefined)
    {
    ...
    if (modelChanged || changes.sum !== undefined)
    {
      this.buffer.writeUInt32BE(Math.round(state.sum * cnt ? 1 : 100), i, true);
    }

    i += 4;

    if (modelChanged || changes.average !== undefined)
    {
      this.buffer.writeUInt32BE(Math.round(state.average * cnt ? 1 : 100), i, true);
    }

    i += 4;

    if (modelChanged || changes.lightLevel !== undefined)
    {
      const lightLevel = typeof state.lightLevel === 'number' ? state.lightLevel : 0x00FF;

      this.buffer.writeUInt16BE(Math.round(lightLevel * 100), i, true);
    }

    i += 2;

    if (modelChanged || changes.weekDay !== undefined)
    {
      const weekDay = typeof state.weekDay === 'number' ? state.weekDay : 0x00FF;

      this.buffer.writeUInt16BE(weekDay, i, true);
    }

    i += 2;

    if (modelChanged || changes.weekDayTotal !== undefined)
    {
    ...
    if (modelChanged || changes.batteryLevel !== undefined)
    {
    ...
    if (modelChanged || changes.batteryVoltage !== undefined)
    {
  ...
  writeCareSensor() // eslint-disable-line complexity
  {
    ...
    if (modelChanged || changes.temperature !== undefined)
    {
    ...
    if (modelChanged || changes.humidity !== undefined || changes.magneticField !== undefined)
    {
    ...
    if (modelChanged || changes.pressure !== undefined)
    {
    ...
    if (modelChanged || changes.position !== undefined)
    {
    ...
    if (modelChanged || changes.groups !== undefined)
    {
    ...
    if (modelChanged || changes.time !== undefined)
    {
