...
class Device
{
  ...
  writeEnergyMeter()
  {
    ...
    {
      this.buffer.writeUInt16BE(Math.round(state.lightLevel * 100), 44, true);
    }

    if (modelChanged || changes.weekDay)
    {
      this.buffer.writeUInt16BE(changes.weekDay || 0, 44, true);
    }

    if (modelChanged || changes.weekDayTotal)
    {
      this.buffer.writeUInt32BE(changes.weekDayTotal || 0, 46, true);
    }

    if (modelChanged || changes.batteryLevel)
    {
      this.buffer.writeUInt16BE(Math.round(state.batteryLevel || 0), 50, true);
    }
    ...
    if (modelChanged || changes.batteryVoltage)
    {
      ...
      this.buffer.writeUInt16BE(Math.round(batteryVoltage * 100), 52, true);
    }
  }
  ...
  writeCareSensor() // eslint-disable-line complexity
  {
    ...
    if ((modelChanged || changes.humidity) && typeof state.humidity === 'number')
    {
      value2 = state.humidity * 100;
    }
    else if ((modelChanged || changes.magneticField) && typeof state.magneticField === 'number')
    {
      value2 = state.magneticField;
    }

    this.buffer.writeUInt16BE(value2, 36, true);
